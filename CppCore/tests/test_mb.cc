...
TEST_CASE("MullerBrown Function properties", "[MullerBrown]") {
  ...
  SECTION("Value at first minimum") {
    x = {-0.558, 1.442};
    REQUIRE_THAT(mullerBrown(x),
                 Catch::Matchers::WithinAbs(-146.69948920058778, 1e-4));
  }

  SECTION("Value at second minimum") {
    x = {0.623, 0.028};
    REQUIRE_THAT(mullerBrown(x),
                 Catch::Matchers::WithinAbs(-108.16665005353302, 1e-4));
  }

  SECTION("Value at third minimum") {
    x = {-0.050, 0.466};
    REQUIRE_THAT(mullerBrown(x),
                 Catch::Matchers::WithinAbs(-80.76746772526472, 1e-4));
  }

  SECTION("Value at first saddle point") {
    x = {0.212, 0.293};
    REQUIRE_THAT(mullerBrown(x),
                 Catch::Matchers::WithinAbs(-72.24891965936473, 1e-4));
  }

  SECTION("Value at second saddle point") {
    x = {-0.822, 0.624};
    REQUIRE_THAT(mullerBrown(x),
                 Catch::Matchers::WithinAbs(-40.66484530104902, 1e-4));
  }
  ...
  SECTION("Value at arbitrary points") {
    x = {-1.050, 0.466};
    REQUIRE_THAT(mullerBrown(x),
                 Catch::Matchers::WithinAbs(-26.4116181673, 1e-4));
    REQUIRE_THAT(mullerBrown({1.623, .38}),
