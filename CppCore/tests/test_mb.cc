...
TEST_CASE("MullerBrown Function properties", "[MullerBrown]") {
    ...
    SECTION("Value at first minimum") {
        x = {-0.558, 1.442};
        REQUIRE_THAT(mullerBrown(x), Catch::Matchers::WithinAbs(-146.69948920058778, 1e-4));
    }

    SECTION("Value at second minimum") {
        x = {0.623, 0.028};
        REQUIRE_THAT(mullerBrown(x), Catch::Matchers::WithinAbs(-108.16665005353302, 1e-4));
    }

    SECTION("Value at third minimum") {
        x = {-0.050, 0.466};
        REQUIRE_THAT(mullerBrown(x), Catch::Matchers::WithinAbs(-80.76746772526472, 1e-4));
    }

    SECTION("Value at first saddle point") {
        x = {0.212, 0.293};
        REQUIRE_THAT(mullerBrown(x), Catch::Matchers::WithinAbs(-72.24891965936473, 1e-4));
    }

    SECTION("Value at second saddle point") {
        x = {-0.822, 0.624};
        REQUIRE_THAT(mullerBrown(x), Catch::Matchers::WithinAbs(-40.66484530104902, 1e-4));
    }

    // TODO(rgoswami): Look into this, though it should be 0, doesn't seem to be the case
    // SECTION("Gradient at first minimum") {
    //     x = {-0.558, 1.442};
    ...
    //     REQUIRE_THAT(hess(0, 0), Catch::Matchers::WithinAbs(11191.4224014, 1e-4));
    //     REQUIRE_THAT(hess(0, 1), Catch::Matchers::WithinAbs(2421.639873505, 1e-4));
    //     REQUIRE_THAT(hess(1, 0), Catch::Matchers::WithinAbs(2421.6398735, 1e-4));
    //     REQUIRE_THAT(hess(1, 1), Catch::Matchers::WithinAbs(613.918945312, 1e-4));
    // }
